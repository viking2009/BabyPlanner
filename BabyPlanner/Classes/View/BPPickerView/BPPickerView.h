...
@protocol BPPickerViewDataSource, BPPickerViewDelegate;

@interface BPPickerView : UIView

@property(nonatomic, weak) id<BPPickerViewDataSource> dataSource;                // default is nil. weak reference
@property(nonatomic, weak) id<BPPickerViewDelegate> delegate;                  // default is nil. weak reference
...
// TODO: add UI_APPEARANCE_SELECTOR

@end


@protocol BPPickerViewDataSource<NSObject>
@required
...
@protocol BPPickerViewDelegate<NSObject>
@optional
...
- (NSAttributedString *)pickerView:(BPPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component NS_AVAILABLE_IOS(6_0); // attributed title is favored if both methods are implemented
